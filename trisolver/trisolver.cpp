/*
 ...
 Date:      2024-02-03
 Purpose:	A program reads an input from the user and calculate
			the missing values of the triangles
*/
#include <iostream>
#include <string>
#include <sstream>
...
int main()
{
	...
	while (true) {
		...
		if (tringle_type == "SSS")
		{
			// Check for impossible triangle based on side lengths
			if ((a + b <= c) || (a + c <= b) || (b + c <= a))
			{
				cout << "Impossible triangle" << endl;
				continue;  // Skip further calculations for this input
			}
			...
			// Check if angles sum up to 180 degrees (allowing for floating-point imprecision)
			if (fabs(alpha_deg + beta_deg + gamma_deg - 180) > EPSILON)
			{
				cout << "Impossible triangle: Angles do not sum up to 180 degrees." << endl;
				continue;  // Skip further calculations for this input
			}
